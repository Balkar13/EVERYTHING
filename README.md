# 🛠️ Ethical Hacking Resources Hub

A curated collection of the **best tools, websites, GitHub repositories, wordlists, labs, and learning resources** for penetration testing and cyber security.

Useful for beginners → advanced pentesters.
Continuously updated 🔥

---

## 📌 Table of Contents

* [💻 Essential Tools](#-essential-tools)
* [🌐 Web Pentesting Resources](#-web-pentesting-resources)
* [🧰 Wordlists & Payloads](#-wordlists--payloads)
* [🔎 Recon & OSINT](#-recon--osint)
* [🔓 Exploitation Tools](#-exploitation-tools)
* [🏴‍☠️ CTF & Labs](#️-ctf--labs)
* [📚 Books & Learning](#-books--learning)
* [📝 Notes](#-notes)

---

# 💻 Essential Tools

### 🐧 *System / Terminal Tools*

| Tool            | Description                      | Link                                                         |
| --------------- | -------------------------------- | ------------------------------------------------------------ |
| **Kali Linux**  | Industry-standard pentesting OS  | [https://www.kali.org](https://www.kali.org)                 |
| **Parrot OS**   | Lightweight pentesting OS        | [https://parrotlinux.org](https://parrotlinux.org)           |
| **Netcat (nc)** | Networking Swiss army knife      | [https://nc110.sourceforge.io](https://nc110.sourceforge.io) |
| **Nmap**        | Port Scanning, Service Detection | [https://nmap.org](https://nmap.org)                         |
| **JOHN**        | CRACKING PASSWORDS               | [https://www.openwall.com/john/](https://www.openwall.com/john/)        |
---

# 🌐 Web Pentesting Resources

### 🔥 *Web Tools*

| Tool           | Usage                           | Link                                                                               |
| -------------- | ------------------------------- | ---------------------------------------------------------------------------------- |
| **Burp Suite** | Web Proxy, Scanner, Fuzzer      | [https://portswigger.net/burp](https://portswigger.net/burp)                       |
| **OWASP ZAP**  | Open source alternative to Burp | [https://www.zaproxy.org](https://www.zaproxy.org)                                 |
| **SQLMap**     | Automated SQL Injection         | [https://github.com/sqlmapproject/sqlmap](https://github.com/sqlmapproject/sqlmap) |
| **XSStrike**   | Advanced XSS tool               | [https://github.com/s0md3v/XSStrike](https://github.com/s0md3v/XSStrike)           |
| **GoBuster**      | Directory Brute-Forcing                      | [https://github.com/OJ/gobuster](https://github.com/OJ/gobuster)               |
| **Hackbar**      | Manual SQLi-Exploitation                      | [https://github.com/PhHitachi/HackBar](https://github.com/PhHitachi/HackBar)               |
| **Ghidra**      | NSA reverse engineering suite                      | [https://ghidra-sre.org](https://ghidra-sre.org)               |
| **x64dbg**      | Powerful Windows debugger                     | [https://x64dbg.com](https://x64dbg.com)               |
---

# 🧰 Wordlists & Payloads

### 📂 *Top Wordlists*

| Repository               | Link                                                                                                       |
| ------------------------ | ---------------------------------------------------------------------------------------------------------- |
| **SecLists (must-have)** | [https://github.com/danielmiessler/SecLists](https://github.com/danielmiessler/SecLists)                   |
| **PayloadAllTheThings**  | [https://github.com/swisskyrepo/PayloadsAllTheThings](https://github.com/swisskyrepo/PayloadsAllTheThings) |
| **FuzzDB**               | [https://github.com/fuzzdb-project/fuzzdb](https://github.com/fuzzdb-project/fuzzdb)                       |

---

# 🔎 Recon & OSINT

### 🔍 *Recon Tools*

| Tool            | Description                   | Link                                                                                           |
| --------------- | ----------------------------- | ---------------------------------------------------------------------------------------------- |
| **Amass**       | Subdomain Enumeration         | [https://github.com/owasp-amass/amass](https://github.com/owasp-amass/amass)                   |
| **Subfinder**   | Fast passive subdomain finder | [https://github.com/projectdiscovery/subfinder](https://github.com/projectdiscovery/subfinder) |
| **Assetfinder** | Bug bounty recon              | [https://github.com/tomnomnom/assetfinder](https://github.com/tomnomnom/assetfinder)           |

### 🕵️ *OSINT Websites*

| Website                                    | Purpose                            |
| ------------------------------------------ | ---------------------------------- |
| [https://crt.sh](https://crt.sh)           | Find SSL Certificates & subdomains |
| [https://shodan.io](https://shodan.io)     | Internet device search engine      |
| [https://hunter.io](https://hunter.io)     | Email discovery                    |
| [https://archive.org](https://archive.org) | Wayback Machine                    |

---

# 🔓 Exploitation Tools

### 💥 *Network & System Exploitation*

| Tool                     | Description                | Link                                                                       |
| ------------------------ | -------------------------- | -------------------------------------------------------------------------- |
| **Metasploit Framework** | Exploitation framework     | [https://metasploit.help.rapid7.com](https://metasploit.help.rapid7.com)   |
| **Responder**            | LLMNR/NBT-NS poisoner      | [https://github.com/lgandx/Responder](https://github.com/lgandx/Responder) |
| **Impacket**             | SMB, MSSQL, Kerberos tools | [https://github.com/fortra/impacket](https://github.com/fortra/impacket)   |

---

# 🏴‍☠️ CTF & Labs

### 🎮 *Practice Labs*

| Platform                | Link                                                                         |
| ----------------------- | ---------------------------------------------------------------------------- |
| **TryHackMe**           | [https://tryhackme.com](https://tryhackme.com)                               |
| **Hack The Box**        | [https://hackthebox.com](https://hackthebox.com)                             |
| **PortSwigger Academy** | [https://portswigger.net/web-security](https://portswigger.net/web-security) |
| **Try2Hack.me**        | [https://try2hack.me/ukoly](https://try2hack.me/ukoly)                         |
| **PicoCTF**        | [https://play.picoctf.org/](https://play.picoctf.org/)                         |
| **CTFlearn**        | [https://ctflearn.com/](https://ctflearn.com/)                         |
| **CyberDefenders**        | [https://cyberdefenders.org/dashboard/](https://cyberdefenders.org/dashboard/)                         |
| **RootMe**        | [https://www.root-me.org/?page=news&lang=fr](https://www.root-me.org/?page=news&lang=fr)                         |
| **Xmass-RootMe**        | [https://ctf.xmas.root-me.org/](https://ctf.xmas.root-me.org/)                         |
| **Over The Wire**        | [https://overthewire.org/wargames/](https://overthewire.org/wargames/)                         |
---

# 📚 Books & Learning

### 📘 *Top Books for Hackers*

| Book                                  | Link                                                   |
| ------------------------------------- | ------------------------------------------------------ |
| **Black-Hat-Python** | [https://balkarsidhu.xyz/files/black-hat-python-python-programming-for-hackers-and-pentesters.pdf](https://balkarsidhu.xyz/files/black-hat-python-python-programming-for-hackers-and-pentesters.pdf) |
| **The Art of Exploitation**           | [https://balkarsidhu.xyz/files/Hacking%20-%20The%20Art%20of%20Exploitation,%202nd%20Edition%20by%20Jon%20Erickson.pdf](https://balkarsidhu.xyz/files/Hacking%20-%20The%20Art%20of%20Exploitation,%202nd%20Edition%20by%20Jon%20Erickson.pdf)           |
| **Reversing:Secrets of Reverse Engineering**        | [https://balkarsidhu.xyz/files/Reversing-Secrets%20of%20Reverse%20Engineering.pdf](https://balkarsidhu.xyz/files/Reversing-Secrets%20of%20Reverse%20Engineering.pdf)           |

---


---
